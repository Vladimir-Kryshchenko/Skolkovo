...
const (
	...
	icoPulse     = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="2" width="20" height="8" rx="2"/><rect x="2" y="14" width="20" height="8" rx="2"/><line x1="6" y1="6" x2="6.01" y2="6"/><line x1="6" y1="18" x2="6.01" y2="18"/></svg>`
...
` + navItem("/ai/models", icoCpu, "ИИ-модели", "Провайдеры и ключи LLM (Qwen, OpenAI, Anthropic, Custom)") +
	...
	navItem("/jobs", icoClock, "Расписание и журнал", "Периодичность проверок по источникам, журнал запусков, расход токенов ИИ") +
	navItem("/tenants", icoBriefcase, "Тенанты и токены", "Организации-заказчики: MCP API-ключи и токены Telegram-ботов (по тенанту)") +
	navItem("/health", icoPulse, "Диагностика", "Состояние подсистем: БД, поиск, ИИ, источники, прокси — и как чинить ошибки") +
	`</nav>
