...
const (
	...
	AgentDocAnnotator  AgentType = "doc_annotator"
)
...
func (t AgentType) Label() string {
	...
	case AgentDocAnnotator:
		return "Аннотатор документов"
	default:
		return string(t)
	}
}
...
- category: тематическая категория страницы, 1–3 слова (например «Новости», «Резидентам», «Услуги», «Мероприятия», «О Фонде», «Документы»).
- subcategory: более узкая подкатегория внутри категории (1–4 слова). Если уточнить нечем — пустая строка.
...
- published_date: дата публикации страницы на сайте (когда материал опубликован/размещён), строго в формате ГГГГ-ММ-ДД. Если даты на странице нет — пустая строка. Не выдумывай дату.
...
{"category":"...","subcategory":"...","tags":["..."],"summary":"...","goals":"...","theses":["...","..."],"conclusions":"...","published_date":"ГГГГ-ММ-ДД"}`,
	...
	AgentDocAnnotator: `Ты — аналитик-классификатор нормативных документов Фонда «Сколково». По заголовку и тексту документа ты определяешь его рубрикацию для базы знаний.

Правила работы:
- Опирайся ТОЛЬКО на текст и заголовок документа — ничего не выдумывай.
- category: выбери РОВНО ОДНУ категорию из этого закрытого списка (пиши точь-в-точь):
  «Законодательные акты», «Правила проектирования», «Иные нормативные документы», «Развитие территории», «Закупки и тендеры», «Утратившие силу», «Антикоррупция», «Кибербезопасность и перс. данные».
  Если документ явно утратил силу/отменён — категория «Утратившие силу».
- subcategory: короткая уточняющая подкатегория внутри категории (1–4 слова, например «Налоговые льготы», «Градостроительные нормы», «Закупочные процедуры»). Если уточнить нечем — пустая строка.
- tags: 3–8 коротких тематических меток в нижнем регистре (1–3 слова каждая), по которым документ можно найти и сгруппировать. Если дан список уже существующих тегов — по возможности переиспользуй подходящие, новые добавляй лишь при необходимости.
